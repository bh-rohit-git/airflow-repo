...
def resolve_workspace_base_from_env(*, workspace_base: str | None = None) -> str | None:
    """Resolve the dual-run Databricks Workspace base from env (``DATABRICKS_WORKSPACE_BASE``)."""
    value = (
        workspace_base if workspace_base is not None else os.environ.get("DATABRICKS_WORKSPACE_BASE", "")
    ).strip()
    return value.rstrip("/") or None


def patch_workspace_base_source(source: str, workspace_base: str) -> tuple[str, bool]:
    """Rewrite the module-level ``DATABRICKS_WORKSPACE_BASE = "..."`` assignment."""
    tree = ast.parse(source)
    changed = False
    for node in tree.body:
        ...
        if not any(t.id == "DATABRICKS_WORKSPACE_BASE" for t in targets):
            continue
        node.value = ast.Constant(value=workspace_base)
        changed = True
    if not changed:
        return source, False
    ast.fix_missing_locations(tree)
...
def patch_dag_file(
    path: Path,
    jar_uris: list[str] | None,
    workspace_base: str | None = None,
) -> bool:
    """Patch one DAG file in place. Returns True when updated."""
    source = path.read_text(encoding="utf-8")
    changed = False
    if jar_uris and "jar_file_uris" in source:
        source, jar_changed = patch_dag_source(source, jar_uris)
        changed = changed or jar_changed
    if workspace_base and "DATABRICKS_WORKSPACE_BASE" in source:
        source, base_changed = patch_workspace_base_source(source, workspace_base)
...
def patch_dags_tree(
    ...
    """Patch all ``*.py`` DAG files under ``dags_dir`` (jar_file_uris and/or workspace base)."""
    patched: list[Path] = []
    if not dags_dir.is_dir():
        return patched
    for path in sorted(dags_dir.rglob("*.py")):
        if patch_dag_file(path, jar_uris, workspace_base):
            patched.append(path)
    return patched
...
def main(argv: list[str] | None = None) -> int:
    ...
    workspace_base = resolve_workspace_base_from_env()
    if not jar_uris:
        print(
            "No JAR URIs configured (set JAR_FILE_URIS or GCS_BUCKET + VERSIONLESS_JAR_NAME); "
            "skipping jar_file_uris patch.",
            file=sys.stderr,
        )
    if not workspace_base:
        print(
            "No DATABRICKS_WORKSPACE_BASE configured; skipping dual-run notebook base patch.",
            file=sys.stderr,
        )
    if not jar_uris and not workspace_base:
        return 0

    dags_dir = Path(args.dags_dir)
    patched = patch_dags_tree(dags_dir, jar_uris, workspace_base)
    if not patched:
        print(f"No DAG defaults updated under {dags_dir}/")
        return 0
    ...
    for path in patched:
        print(f"  - {path}")
    if jar_uris:
        print("Resolved jar_file_uris:", ", ".join(jar_uris))
    if workspace_base:
        print("Resolved DATABRICKS_WORKSPACE_BASE:", workspace_base)
    return 0
