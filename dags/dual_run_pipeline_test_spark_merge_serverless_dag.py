...
with DAG(
    ...
    compare_dualrun_outputs = DualRunDatabricksSubmitRunOperator(
        ...
        json=_serverless_notebook_task(
            ...
            {'dual_run_id': "{{ (dag_run.conf or {}).get('dual_run_id', dag_run.run_id) }}", 'project_name': "{{ (dag_run.conf or {}).get('project_name', dag.dag_id) }}", 'group_id': '{% set _v = (dag_run.conf or {}).get("group_id") or params.get("group_id", "com.example.merge.Main:48f9b0e6787e") %}{{ _v if _v is string else (_v | tojson) }}', 'report_root': '{% set _v = (dag_run.conf or {}).get("report_root") or params.get("report_root", "gs://{{ var.value.dual_run_bucket }}/dual_run/reports") %}{{ _v if _v is string else (_v | tojson) }}', 'compare_pairs_json': '{% set _v = (dag_run.conf or {}).get("compare_pairs_json") or params.get("compare_pairs_json", "[{\\"left_ref\\":\\"databricks-migrate-activity.schema1.customer_events_dual_clone_1\\",\\"right_ref\\":\\"databricks-migrate-activity.schema1.customer_events_dual_clone_2\\",\\"label\\":\\"customer_events\\"}]") %}{{ _v if _v is string else (_v | tojson) }}', 'compare_left_ref_0': '{% set _v = (dag_run.conf or {}).get("compare_left_ref_0") or params.get("compare_left_ref_0", "databricks-migrate-activity.schema1.customer_events_dual_clone_1") %}{{ _v if _v is string else (_v | tojson) }}', 'compare_right_ref_0': '{% set _v = (dag_run.conf or {}).get("compare_right_ref_0") or params.get("compare_right_ref_0", "databricks-migrate-activity.schema1.customer_events_dual_clone_2") %}{{ _v if _v is string else (_v | tojson) }}'},
        ),
    )
