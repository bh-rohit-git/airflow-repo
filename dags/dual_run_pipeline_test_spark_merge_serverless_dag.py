...
with DAG(
    ...
    ensure_dualrun_clones = DualRunDatabricksSubmitRunOperator(
        ...
        json=_serverless_notebook_task(
            ...
            {'clone_plan_json': '{% set _v = (dag_run.conf or {}).get("clone_plan_json", "{}") %}{{ _v if _v is string else (_v | tojson) }}', 'clone_suffix': '{% set _v = (dag_run.conf or {}).get("clone_suffix") or (dag_run.conf or {}).get("source_branch_conf", {}).get("clone_suffix") or params.get("clone_suffix", "_dual_clone_") %}{{ _v if _v is string else (_v | tojson) }}', 'catalog': '{% set _v = (dag_run.conf or {}).get("catalog") or (dag_run.conf or {}).get("source_branch_conf", {}).get("catalog") or params.get("catalog", "databricks-migrate-activity") %}{{ _v if _v is string else (_v | tojson) }}', 'merge-key': '{% set _v = (dag_run.conf or {}).get("merge-key") or (dag_run.conf or {}).get("source_branch_conf", {}).get("merge-key") or params.get("merge-key", "") %}{{ _v if _v is string else (_v | tojson) }}', 'input-path': '{% set _v = (dag_run.conf or {}).get("input-path") or (dag_run.conf or {}).get("source_branch_conf", {}).get("input-path") or params.get("input-path", "") %}{{ _v if _v is string else (_v | tojson) }}', 'target-table': '{% set _v = (dag_run.conf or {}).get("target-table") or (dag_run.conf or {}).get("source_branch_conf", {}).get("target-table") or params.get("target-table", "") %}{{ _v if _v is string else (_v | tojson) }}'},
        ),
    )
    ...
    compare_dualrun_outputs = DualRunDatabricksSubmitRunOperator(
        ...
        json=_serverless_notebook_task(
            ...
            {'dual_run_id': "{{ (dag_run.conf or {}).get('dual_run_id', dag_run.run_id) }}", 'project_name': "{{ (dag_run.conf or {}).get('project_name', dag.dag_id) }}", 'group_id': '{% set _v = (dag_run.conf or {}).get("group_id") or params.get("group_id", "com.example.merge.Main:1098cf1a3b95") %}{{ _v if _v is string else (_v | tojson) }}', 'report_root': '{% set _v = (dag_run.conf or {}).get("report_root") or params.get("report_root", "gs://{{ var.value.dual_run_bucket }}/dual_run/reports") %}{{ _v if _v is string else (_v | tojson) }}', 'compare_pairs_json': '{% set _v = (dag_run.conf or {}).get("compare_pairs_json") or params.get("compare_pairs_json", "[{\\"left_ref\\":\\"databricks-migrate-activity.schema1.customer_events_dual_clone_1\\",\\"right_ref\\":\\"databricks-migrate-activity.schema1.customer_events_dual_clone_2\\",\\"label\\":\\"customer_events\\"}]") %}{{ _v if _v is string else (_v | tojson) }}'},
        ),
    )
