...
with DAG(
    ...
    mock_source_data = DatabricksSubmitRunOperator(
        ...
        json=_serverless_notebook_task(
            ...
            {'source_specs_json': "{{ (dag_run.conf or {}).get('source_specs_json', '[]') }}", 'source_clone_suffix': '{{ (dag_run.conf or {}).get("source_clone_suffix") or params.get("source_clone_suffix", "_source_clone") }}', 'input-path': '{{ (dag_run.conf or {}).get("input-path") or params.get("input-path", "") }}', 'merge-key': '{{ (dag_run.conf or {}).get("merge-key") or params.get("merge-key", "") }}'},
        ),
    )
