...
with DAG(
    dag_id="dualrun_RawOrderIngestionMain_4eb9a6fdd64e",
    ...
    trigger_source_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id="dualrun_child_unmigrated_RawOrderIngestionMain_4eb9a6fdd64e",
    ...
    trigger_migrated_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id="dualrun_child_migrated_RawOrderIngestionMain_4eb9a6fdd64e",
    ...
    compare_dualrun_outputs = DualRunDatabricksSubmitRunOperator(
        ...
        json=_serverless_notebook_task(
            ...
            {'dual_run_id': "{{ (dag_run.conf or {}).get('dual_run_id', dag_run.run_id) }}", 'project_name': "{{ (dag_run.conf or {}).get('project_name', dag.dag_id) }}", 'group_id': '{% set _v = (dag_run.conf or {}).get("group_id") or params.get("group_id", "com.example.streaming.RawOrderIngestionMain:4eb9a6fdd64e") %}{{ _v if _v is string else (_v | tojson) }}', 'report_root': '{% set _v = (dag_run.conf or {}).get("report_root") or params.get("report_root", "gs://{{ var.value.dual_run_bucket }}/dual_run/reports") %}{{ _v if _v is string else (_v | tojson) }}', 'compare_pairs_json': '{% set _v = (dag_run.conf or {}).get("compare_pairs_json") or params.get("compare_pairs_json", "[{\\"left_ref\\":\\"databricks-migrate-activity.schema1.bronze_orders_dual_clone_1\\",\\"right_ref\\":\\"databricks-migrate-activity.schema1.bronze_orders_dual_clone_2\\",\\"label\\":\\"bronze_orders\\"}]") %}{{ _v if _v is string else (_v | tojson) }}'},
        ),
    )
