"""Dual-run pipeline for run_raw_order_ingestion_pubsub (generated by bh-migrate).
...
DAG_PARAMS = {
    ...
    "checkpoint-location": Param(default="gs://bh-migrate-poc-bucket/pubsub-setup/checkpoints/bronze_orders_pubsub", type="string"),
...
with DAG(
    dag_id="dualrun_RawOrderIngestionMain_cb0e9d4643d0",
    default_args=default_args,
    description="Dual-run validation for run_raw_order_ingestion_pubsub (com.example.streaming.RawOrderIngestionMain)",
    schedule=None,
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=["dual-run", "bh-migrate", "run_raw_order_ingestion_pubsub"],
    params=DAG_PARAMS,
    ...
    ensure_dualrun_clones = DualRunDatabricksSubmitRunOperator(
        ...
        json=_serverless_notebook_task(
            "ensure_clones",
            f"{DATABRICKS_WORKSPACE_BASE}/run_raw_order_ingestion_pubsub/ensure_clones.py",
            {'clone_plan_json': '{% set _v = (dag_run.conf or {}).get("clone_plan_json", "{}") %}{{ _v if _v is string else (_v | tojson) }}', 'clone_suffix': '{% set _v = (dag_run.conf or {}).get("clone_suffix") or (dag_run.conf or {}).get("source_branch_conf", {}).get("clone_suffix") or params.get("clone_suffix", "_dual_clone_") %}{{ _v if _v is string else (_v | tojson) }}', 'catalog': '{% set _v = (dag_run.conf or {}).get("catalog") or (dag_run.conf or {}).get("source_branch_conf", {}).get("catalog") or params.get("catalog", "databricks-migrate-activity") %}{{ _v if _v is string else (_v | tojson) }}', 'checkpoint-location': '{% set _v = (dag_run.conf or {}).get("checkpoint-location") or (dag_run.conf or {}).get("source_branch_conf", {}).get("checkpoint-location") or params.get("checkpoint-location", "") %}{{ _v if _v is string else (_v | tojson) }}', 'gcp-project': '{% set _v = (dag_run.conf or {}).get("gcp-project") or (dag_run.conf or {}).get("source_branch_conf", {}).get("gcp-project") or params.get("gcp-project", "") %}{{ _v if _v is string else (_v | tojson) }}', 'idle-timeout': '{% set _v = (dag_run.conf or {}).get("idle-timeout") or (dag_run.conf or {}).get("source_branch_conf", {}).get("idle-timeout") or params.get("idle-timeout", "") %}{{ _v if _v is string else (_v | tojson) }}', 'service-credential': '{% set _v = (dag_run.conf or {}).get("service-credential") or (dag_run.conf or {}).get("source_branch_conf", {}).get("service-credential") or params.get("service-credential", "") %}{{ _v if _v is string else (_v | tojson) }}', 'subscription-id': '{% set _v = (dag_run.conf or {}).get("subscription-id") or (dag_run.conf or {}).get("source_branch_conf", {}).get("subscription-id") or params.get("subscription-id", "") %}{{ _v if _v is string else (_v | tojson) }}', 'topic-id': '{% set _v = (dag_run.conf or {}).get("topic-id") or (dag_run.conf or {}).get("source_branch_conf", {}).get("topic-id") or params.get("topic-id", "") %}{{ _v if _v is string else (_v | tojson) }}', 'trigger-interval': '{% set _v = (dag_run.conf or {}).get("trigger-interval") or (dag_run.conf or {}).get("source_branch_conf", {}).get("trigger-interval") or params.get("trigger-interval", "") %}{{ _v if _v is string else (_v | tojson) }}', 'target_table': '{% set _v = (dag_run.conf or {}).get("target_table") or (dag_run.conf or {}).get("source_branch_conf", {}).get("target_table") or params.get("target_table", "") %}{{ _v if _v is string else (_v | tojson) }}'},
        ),
    )
    ...
    mock_source_data = DualRunDatabricksSubmitRunOperator(
        ...
        json=_serverless_notebook_task(
            "mock_source_data",
            f"{DATABRICKS_WORKSPACE_BASE}/run_raw_order_ingestion_pubsub/mock_source_data.py",
    ...
    trigger_source_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id="dualrun_child_unmigrated_RawOrderIngestionMain_cb0e9d4643d0",
    ...
    trigger_migrated_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id="dualrun_child_migrated_RawOrderIngestionMain_cb0e9d4643d0",
    ...
    compare_dualrun_outputs = DualRunDatabricksSubmitRunOperator(
        ...
        json=_serverless_notebook_task(
            "compare_outputs",
            f"{DATABRICKS_WORKSPACE_BASE}/run_raw_order_ingestion_pubsub/compare_outputs.py",
            {'dual_run_id': "{{ (dag_run.conf or {}).get('dual_run_id', dag_run.run_id) }}", 'project_name': "{{ (dag_run.conf or {}).get('project_name', dag.dag_id) }}", 'group_id': '{% set _v = (dag_run.conf or {}).get("group_id") or params.get("group_id", "com.example.streaming.RawOrderIngestionMain:cb0e9d4643d0") %}{{ _v if _v is string else (_v | tojson) }}', 'report_root': '{% set _v = (dag_run.conf or {}).get("report_root") or params.get("report_root", "gs://{{ var.value.dual_run_bucket }}/dual_run/reports") %}{{ _v if _v is string else (_v | tojson) }}', 'compare_pairs_json': '{% set _v = (dag_run.conf or {}).get("compare_pairs_json") or params.get("compare_pairs_json", "[{\\"left_ref\\":\\"databricks-migrate-activity.schema1.bronze_orders_dual_clone_1\\",\\"right_ref\\":\\"databricks-migrate-activity.schema1.bronze_orders_dual_clone_2\\",\\"label\\":\\"bronze_orders\\"}]") %}{{ _v if _v is string else (_v | tojson) }}'},
        ),
    )
