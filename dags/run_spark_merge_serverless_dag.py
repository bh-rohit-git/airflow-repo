...
_DAG_SIZING_PATH = Path(__file__).resolve().parent.parent / 'plugins' / 'run_spark_merge_serverless_dag_cluster_config.yml'
...
if not _DAG_SIZING_PATH.is_file():
    raise FileNotFoundError('run_spark_merge_serverless_dag_cluster_config.yml not found under plugins/. Upload it to gs://<composer_bucket>/plugins/run_spark_merge_serverless_dag_cluster_config.yml.')
...
with DAG(dag_id='run_spark_merge_serverless_dag', default_args=default_args, description='Trigger Spark merge job on Databricks serverless (JAR task)', schedule=None, start_date=datetime(2025, 1, 1), catchup=False, tags=['databricks', 'spark', 'serverless'], params={'input_path': Param(default='gs://databricks-8259550742932689-unitycatalog/8259550742932689/incoming/customer_events_merge.csv', type='string', description='GCS path to the input CSV file or folder'), 'jar_file_uris': Param(default=['/Volumes/databricks-migrate-activity/schema1/jars/spark-merge-job-assembly.jar'], type='array', description='JAR file URIs for the Dataproc Spark job')}) as dag:
    run_spark_merge = DataprocSubmitJobOperator(task_id='run_spark_merge', project_id=GCP_PROJECT_ID, region=GCP_REGION, job={'placement': {'cluster_name': DATAPROC_CLUSTER_NAME}, 'spark_job': {'main_class': 'com.example.merge.Main', 'jar_file_uris': "{{ dag_run.conf.get('jar_file_uris', params.jar_file_uris) }}", 'args': ['--input-path', "{{ dag_run.conf.get('input_path', params.input_path) }}", '--target-table', '`databricks-migrate-activity`.schema1.customer_events', '--merge-key', 'id']}})
