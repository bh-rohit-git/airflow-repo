...
def build_serverless_jar_payload() -> dict:
    ...
    parameters = ['--gcp-project', _conf_or_param('gcp_project'), '--topic-id', _conf_or_param('pubsub_topic'), '--subscription-id', _conf_or_param('pubsub_subscription'), '--target-table', _conf_or_param('bronze_table'), '--checkpoint-location', _conf_or_param('checkpoint_location'), '--trigger-interval', _conf_or_param('trigger_interval'), '--idle-timeout', _conf_or_param('idle_timeout'), '--service-credential', _conf_or_param('pubsub_service_credential')]
...
with DAG(dag_id='migrated_run_raw_order_ingestion_pubsub', description='Serverless JAR: Pub/Sub → bronze_orders (stops after idle timeout)', start_date=datetime(2025, 1, 1), schedule=None, catchup=False, tags=['databricks', 'pubsub', 'streaming', 'serverless'], params={'gcp_project': Param(GCP_PROJECT, type='string', description='GCP project id for Pub/Sub (JAR --gcp-project)'), 'pubsub_topic': Param(PUBSUB_TOPIC, type='string', description='Pub/Sub topic id (JAR --topic-id)'), 'pubsub_subscription': Param(PUBSUB_SUBSCRIPTION, type='string', description='Pub/Sub subscription id (JAR --subscription-id)'), 'bronze_table': Param(BRONZE_TABLE, type='string', description='Bronze Delta target table (JAR --target-table)'), 'checkpoint_location': Param(CHECKPOINT_LOCATION, type='string', description='GCS checkpoint path for structured streaming'), 'trigger_interval': Param(TRIGGER_INTERVAL, type='string', description="Spark micro-batch trigger (e.g. '30 seconds', '1 minute')"), 'idle_timeout': Param(DEFAULT_IDLE_TIMEOUT, type='string', description="Stop Spark after this long with no Pub/Sub input (e.g. '2 minutes', '5 minutes')"), 'pubsub_service_credential': Param(PUBSUB_SERVICE_CREDENTIAL, type='string', description='Unity Catalog service credential name for Pub/Sub auth (JAR --service-credential)'), 'main_class': Param(MAIN_CLASS, type='string', description='Databricks JAR entrypoint main class'), 'uc_jar_path': Param(UC_JAR_PATH, type='string', description='Unity Catalog volume path to the assembly JAR'), 'serverless_env_version': Param(SERVERLESS_ENV_VERSION, type='string', description='Databricks serverless environment version'), 'safety_timeout_seconds': Param(SAFETY_TIMEOUT_SECONDS, type='integer', description='Databricks job run safety timeout in seconds'), 'run_name': Param(RUN_NAME, type='string', description='Databricks jobs/runs/submit run_name'), 'performance_target': Param(PERFORMANCE_TARGET, type='string', description='Serverless performance target (e.g. PERFORMANCE_OPTIMIZED)'), 'jar_file_uris': Param(default=["{{ dag_run.conf.get('uc_jar_path', params.uc_jar_path) }}"], type='array', description='JAR file URIs for the Dataproc Spark job')}, render_template_as_native_obj=True) as dag:
