...
def _dataproc_task_telemetry_log_path() -> str:
    try:
        ...
        import os
        return os.environ.get('DATAPROC_TASK_TELEMETRY_LOG', '/home/airflow/gcs/logs/dataproc_task_telemetry.jsonl')

def _dataproc_task_telemetry_write(event: str, context, *, error: str | None=None) -> None:
    ti = context['ti']
    dag_run = context.get('dag_run')
    record = {'timestamp': datetime.now(timezone.utc).isoformat(), 'event': event, 'dag_id': ti.dag_id, 'task_id': ti.task_id, 'run_id': getattr(dag_run, 'run_id', None) if dag_run else context.get('run_id'), 'try_number': ti.try_number}
...
def dataproc_task_telemetry_on_failure(context) -> None:
    ...
    _dataproc_task_telemetry_write('task_failure', context, error=str(exc) if exc is not None else None)
...
_DAG_SIZING_PATH = Path(__file__).resolve().parent.parent / 'plugins' / 'test_spark_merge_serverless_dag_migrated_cluster_config.yml'
...
if not _DAG_SIZING_PATH.is_file():
    raise FileNotFoundError('test_spark_merge_serverless_dag_migrated_cluster_config.yml not found under plugins/. Upload it to gs://<composer_bucket>/plugins/test_spark_merge_serverless_dag_migrated_cluster_config.yml.')
...
TASK_TELEMETRY_LOG_PATH = _ENV_CONFIG.get('task_telemetry_log_path', '/home/airflow/gcs/logs/dataproc_task_telemetry.jsonl')
SPARK_HISTORY_SERVER_URL = _ENV_CONFIG.get('spark_history_server_url')
PERFORMANCE_REPORT_DIR = _ENV_CONFIG.get('performance_report_dir', '/home/airflow/gcs/logs/performance_reports')
...
analyze_spark_performance = PythonOperator(task_id='analyze_spark_performance', python_callable=analyze_dataproc_spark_job, op_kwargs={'upstream_spark_task_ids': ['run_spark_merge'], 'project_id': GCP_PROJECT_ID, 'region': GCP_REGION, 'cluster_name': DATAPROC_CLUSTER_NAME, 'spark_history_server_url': SPARK_HISTORY_SERVER_URL, 'performance_report_dir': PERFORMANCE_REPORT_DIR})
