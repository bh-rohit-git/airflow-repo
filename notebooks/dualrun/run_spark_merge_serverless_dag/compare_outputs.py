...
dbutils.widgets.text("candidate_id", "", "Candidate ID")
dbutils.widgets.text("project_name", "", "Project name")
dbutils.widgets.text("report_root", "gs://migration-dualrun/reports", "Report root")
...
report_path = (
    f"{report_root}/project={project_name}/candidate={candidate_id}/dual_run_id={dual_run_id}"
)
...
detail_format = (COMPARE_CONFIG.get("report") or {}).get("detail_format") or "delta"
fail_on = (COMPARE_CONFIG.get("runtime") or {}).get("fail_process_on") or ["FAIL", "BLOCKED"]
...
if overall_severity in fail_on:
    raise Exception(f"dual-run compare failed with overall severity {overall_severity}")
